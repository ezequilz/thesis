...
Vectorized numpy pipeline per frame:
  ...
  4. Occlusion via a soft depth buffer: an exponentially depth-weighted
     accumulation yields a per-pixel "front surface" depth; contributions are
     split into a front layer (within a depth band of that surface) and a
     rear layer, each alpha-composited (weighted-OIT style) over the next.

Approximations vs. true sorted alpha blending (gsplat): within a depth band
splats are averaged by weight rather than sorted, and coverage saturates as
1 - exp(-boost * sum(w)). Good enough to judge reconstruction quality; use the
gsplat backend on a CUDA machine for reference-faithful renders.
"""
...
class CpuSplatRenderer:
    ...
    def render_with_depth(self, camera: Camera) -> tuple[np.ndarray, np.ndarray]:
        ...
        # at the clipped footprint edge (no hard square truncation).
        ...
        alphas = (self._alphas[idx0] / (1.0 - w_edge)).astype(np.float32)
        ...
        # Pass 1: soft depth buffer. S1/S0 is a weighted soft-min of depth.
        z_min = float(z.min())
        band_scale = max(self.depth_band_min, self.depth_band_rel * float(np.median(z)))
        k = 4.0 / band_scale
        z_exp = np.exp(-np.minimum(k * (z - z_min), 600.0))  # per splat, not per pixel
        npix = H * W
        S0 = np.zeros(npix)
        S1 = np.zeros(npix)
        n_contrib = 0
        for pix, w, gi in self._contributions(splats, W, H):
            n_contrib += len(pix)
            S0 += np.bincount(pix, weights=w, minlength=npix)
            S1 += np.bincount(pix, weights=w * z_exp[gi], minlength=npix)
        covered = S0 > 0
        z_front = np.full(npix, np.inf)
        with np.errstate(divide="ignore"):
            z_front[covered] = z_min - np.log(S1[covered] / S0[covered]) / k
        z_limit = z_front + np.maximum(self.depth_band_min, self.depth_band_rel * z_front)

        # Pass 2: accumulate a front layer (the visible surface) and a rear
        # layer (whatever is behind it) and composite front-over-rear-over-bg.
        # Front/rear share one bincount by offsetting rear pixel indices.
        acc = np.zeros((2, 4, npix))  # [layer, (w, r, g, b), pixel]
        for pix, w, gi in self._contributions(splats, W, H):
            lpix = pix + np.where(z[gi] <= z_limit[pix], 0, npix)
            acc[:, 0] += np.bincount(lpix, weights=w, minlength=2 * npix).reshape(2, npix)
            for ch in range(3):
                acc[:, ch + 1] += np.bincount(
                    lpix, weights=w * colors[gi, ch], minlength=2 * npix
                ).reshape(2, npix)
    ...
    def _composite(self, acc: np.ndarray, npix: int) -> np.ndarray:
        ...
        out[:] = self.background
        for layer in (1, 0):  # rear first, front over it
            wsum = acc[layer, 0]
            hit = wsum > 0
