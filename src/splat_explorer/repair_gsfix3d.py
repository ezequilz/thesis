...
``apply_until`` keeps the Gaussian tensors on the GPU and repeats the
paper 20-iter inner loop so Stop still works for research. Each chunk
is the paper inner loop; only a CPU copy is taken for the dashboard.
``Replay all views`` then runs ``kf_iters=50`` shuffled passes over the
repaired images (our stand-in for the paper's augmented captured
dataset — we do not have the original RGB-D capture).
"""
...
class GsplatGsfix3dRepair:
    ...
    def apply_until(
        ...
        *,
        ...
        Gaussians stay on the GPU across chunks. Set ``max_chunks=0`` to keep
        going until Stop / deadline (research).
        """
        import time
    ...
    def _step_chunk(
        ...
        """One paper 20-iter pass. Reuses ``state['ctx']`` so Gaussians stay on GPU."""
    ...
    def _gpu_advance(self, ctx: dict[str, Any], scene: GaussianScene) -> dict[str, Any]:
        ...
        n0 = ctx["n0"]
        w, h = ctx["w"], ctx["h"]
        orig_w, orig_h = ctx["orig_w"], ctx["orig_h"]
